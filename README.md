...
* `AWS_INSTANCES_COUNT` - the number of proxy servers to create. Default is 1. You can set it [up to 20](https://aws.amazon.com/ec2/faqs/#How_many_instances_can_I_run_in_Amazon_EC2).
* `AWS_DEFAULT_REGION` - region of instances (proxy servers) where they will be created. Default is `eu-central-1`. Check [available regions here](https://docs.aws.amazon.com/general/latest/gr/rande.html#ec2_region). Keep in mind that `AWS_INSTANCE_AMI` should match AWS_DEFAULT_REGION. You can find required AWS_INSTANCE_AMI for a specific region here: <https://eu-central-1.console.aws.amazon.com/ec2/v2/home#LaunchInstanceWizard:>
* `PROXY_TYPE` - type of proxy server. HTTP/HTTPS anonymous proxy as default (`http`). Change to `socks` (socks5) if you need it.
* `PROXY_PORT` - port of proxy server. Default is `46666`.
